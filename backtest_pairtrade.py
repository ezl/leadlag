...
def trade(SPY, VXX, spy_yield_low=0, spy_yield_high=10):
    ...
    SPY_open = SPY[0]
    VXX_open = VXX[0]
    SPY_yields = np.log(SPY / SPY_open)
    VXX_yields = np.log(VXX/  VXX_open)
    window = 20
    if window > SPY_yields.shape[0]:
        print "*AA GSLAKDG HASDKLG HASLDK GHASLDK GHASDLGKHASDGKLSADG"
        return 0,0,0,0
    VXX_yield_moving_average = np.hstack((np.zeros(window - 1), np.convolve(np.ones(window), VXX_yields, "valid")))
    SPY_yield_moving_average = np.hstack((np.zeros(window - 1), np.convolve(np.ones(window), SPY_yields, "valid")))
    trade = False
    cost_per_share = .03
    trade_count = 0
